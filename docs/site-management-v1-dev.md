...
桌面端用于后台配置、看板、批量导入和归档导出。布局采用顶部栏 + 左侧导航 + 内容区：
...
| 角色 | 主要职责 |
| --- | --- |
| 管理员 | 系统配置、用户管理、基础数据、图纸、导入导出、审计、备份检查 |
...
管理员上传区域图纸，支持图片和 PDF。PDF 可按页生成预览图，供现场人员查询和管理人员维护版本。
...
### 12.8 DrawingRevision

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| id | uuid | 主键 |
| drawingId | uuid | 所属图纸 |
| revisionNo | string | 版本号 |
| fileKey | string | 原始文件对象存储 Key |
| coverPreviewKey | string | 封面预览图对象存储 Key |
| pageCount | number | 页数 |
| uploadedBy | uuid | 上传人 |
| uploadedAt | datetime | 上传时间 |
| isCurrent | boolean | 是否当前版本 |

### 12.8.1 DrawingRevisionPage

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| id | uuid | 主键 |
| drawingRevisionId | uuid | 所属图纸版本 |
| pageNumber | number | 页码，从 1 开始 |
| previewKey | string | 当前页预览图对象存储 Key |
| width | number | 预览图原始宽度 |
| height | number | 预览图原始高度 |
...
- `GET /drawings`：图纸列表。
- `POST /drawings`：创建图纸。
- `POST /drawings/:id/revisions`：上传新版本。
- `GET /drawings/:id/revisions`：版本列表。
- `GET /drawing-revisions/:id/preview`：获取预览地址。
- `GET /drawing-revisions/:id/pages`：获取图纸版本的分页预览地址、页码、宽度和高度。
- `PATCH /drawing-revisions/:id/current`：设为当前版本。
...
- 图纸上传图片和 PDF 均可预览。
- 上传新版图纸后，管理员可查看版本列表并设置当前版本。
- 多页 PDF 的每一页都有独立预览图、宽度和高度。
